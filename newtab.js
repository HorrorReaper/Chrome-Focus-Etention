...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("startButton").onclick = () => {
    if (state.enabled) {
      // running -> PAUSE
      chrome.runtime.sendMessage({ type: "pauseTimer" }, () => updateState());
    } else if (state.paused && state.pausedRemaining > 0) {
      // paused -> RESUME
      chrome.runtime.sendMessage({ type: "resumeTimer" }, () => updateState());
    } else {
      // idle -> START
      const minutes = state.pomodoroMode
        ? state.pomodoroWork
        : parseInt(document.getElementById("timerInput").value) || 0;
      chrome.runtime.sendMessage({ type: "startTimer", minutes }, () =>
        updateState()
      );
    }
  };
  ...
  document.getElementById("saveSettings").onclick = () => {
    ...
    if (newActiveList !== state.activeList) {
